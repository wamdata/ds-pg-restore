...
from ds_pg_restore.logger import get_logger
...
def check_s3_file_modifications(
    ...
    try:
        ...
        if (
            ...
            logger.info(f"File not modified: s3://{bucket_name}/{key}")
            return None
        ...
        if isinstance(e, ClientError) and e.response["Error"]["Code"] == "304":
            logger.info(f"File not modified: s3://{bucket_name}/{key}")
            return None
        logger.error(f"Failed to get metadata for s3://{bucket_name}/{key}: {str(e)}")
        raise
...
def download_file_from_s3(
    ...
    logger.info(f"Starting download of s3://{bucket_name}/{key} to {filename}")
    ...
    try:
        ...
        logger.info(f"Successfully downloaded s3://{bucket_name}/{key} to {filename}")
    except Exception as e:
        logger.error(
            f"Failed to download s3://{bucket_name}/{key} to {filename}: {str(e)}"
        )
        raise
